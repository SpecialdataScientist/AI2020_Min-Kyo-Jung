...
def read_data(filename):
    ...
    for line in open(filename):
        ...
        word, tag, chunk, label = line.split(' ')   # divide ' ' word, tag, chunk, label
        word = word.lower()
...
def make_vocab(train):
    ...
    for sent in train:
        ...
        for w, t, l in zip(words, tags, labels):
            if w not in word_dict:
                word_dict[w] = 0    # first input word case
            word_dict[w] += 1       # counting
    ...
    # index set
    word2id = {w: i + 2 for i, (w, c) in enumerate(sorted_words)} # use pad, unk
    tag2id = {w: i + 2 for i, (w, c) in enumerate(sorted_tags)}
    label2id = {w: i for i, (w, c) in enumerate(sorted_labels)} # not use pad, unk

    # update pad to 0, unk to 1
    # 2 ~ word, 2 ~ tag
    word2id['<PAD>'] = 0
    word2id['<UNK>'] = 1
...
def build_pretrained_lookup_table(word2id, pretrained_emb_file, embedding_dim=100):
    ...
    # embedding set random
    # vocab size == embedding
    embedding = np.random.uniform(-scale, scale, [vocab_size, embedding_dim]).astype(np.float32)

    # if embedding_dic has vocab word to replace pretrained embedding
...
if __name__ == "__main__":
    ...
    # make vocab (use traindata)
    word2id, tag2id, label2id = make_vocab(traindata)

    # data to idx (0-word, 1-tag, 2-label, 3-sentence_num)
